...
/// [`export_extension!`]. Emits the `#[unsafe(no_mangle)] extern
/// "Rust" fn jackdaw_register_reflect_types_v1(&mut TypeRegistry)`
/// symbol the loader calls after `dlopen`.
///
/// The body `include!`s `$OUT_DIR/__jackdaw_reflect_types.rs`,
/// generated by the user crate's `build.rs`. The generated file
/// always defines a `fn __jackdaw_register_reflect_types(&mut
/// TypeRegistry)` — even if empty — so the emission is
/// unconditional. If a crate ships without the build script (older
/// scaffolds, hand-maintained extensions), a fallback cfg emits an
/// empty body.
///
/// We gate the `include!` on a `jackdaw_build_has_generated_reflect`
/// cfg that the build.rs sets via `println!("cargo:rustc-cfg=...")`.
/// When absent, the fallback no-op body compiles.
#[doc(hidden)]
#[macro_export]
macro_rules! __jackdaw_emit_reflect_register_symbol {
    () => {
        // Suppress `unexpected_cfgs` for crates that don't ship a
        // build.rs (i.e., extensions not using the
        // reflect-auto-register pattern). rustc's lint otherwise
        // fires because it doesn't know the cfg name.
