...
pub fn spawn_node_ui_for_new_graph_nodes(
    ...
    for (data_entity, graph_node, parent) in added.iter() {
        // If a view already exists for this entity (e.g. the demo created
        // one by hand), don't double-spawn.
        if views.iter().any(|v| v.0 == data_entity) {
            continue;
        }

        let Some(parent) = parent else {
            // Node not yet attached to a graph; retry next frame once the
            // ChildOf relation is in place.
            continue;
        };
